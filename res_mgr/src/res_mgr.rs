...
impl ResMgr {
    ...
    pub fn register<T: Res + 'static>(
        ...
        let min = &mut self.min_capacity;
        self.tables
            .entry(TypeId::of::<T>())
            .and_modify(|e| {
                let c = &mut e.caches()[group_i];
                *weight -= c.max_capacity - c.min_capacity;
                *min -= c.min_capacity;
            ...
            .or_insert({
                let mut e = Box::new(ResMap::<T>::default());
                let c = &mut e.caches()[group_i];
    ...
    /// 放入资源
    #[inline]
    pub fn push<T: Res + 'static>(&mut self, res: Share<T>) {
        match self.tables.get_mut(&TypeId::of::<T>()) {
            Some(r) => match <dyn ResCollect>::downcast_mut::<ResMap<T>>(r.as_mut()) {
                Some(map) => map.push(res),
        ...
        if self.temp.len() > 0 {
            if free_size > 0 {
                ...
                for (tid, index, capacity) in &self.temp {
                    ...
                    c.cur_capacity = *capacity + ((c.size - *capacity) as f32 * free / overflow) as usize;
...
mod test_mod {
    ...
    pub fn test() {
        ...
        res_mgr.push::<R1>(Share::new(R1(123456789, 262144, 0)));
        res_mgr.push::<R3>(Share::new(R3(3902250154, 32)));
        res_mgr.push::<R3>(Share::new(R3(1519695964, 16)));
        res_mgr.push::<R6>(Share::new(R6(2905594028, 8)));
        res_mgr.push::<R6>(Share::new(R6(3006512311, 8)));
        res_mgr.push::<R4>(Share::new(R4(308248423, 8)));
        res_mgr.push::<R4>(Share::new(R4(2591543091, 8)));
        res_mgr.push::<R9>(Share::new(R9(2106312588, 8)));
        res_mgr.push::<R9>(Share::new(R9(796366362, 8)));
        res_mgr.push::<R11>(Share::new(R11(3879787636, 8)));
        res_mgr.push::<R12>(Share::new(R12(1145791972, 8)));
        println!(
        ...
        res_mgr.push::<R9>(Share::new(R9(2492188942, 8)));
        res_mgr.push::<R9>(Share::new(R9(4246038113, 8)));
        res_mgr.push::<R9>(Share::new(R9(2564464371, 8)));
        res_mgr.push::<R9>(Share::new(R9(1601737751, 8)));

        res_mgr.push::<R1>(Share::new(R1(1, 131072, 0)));
        res_mgr.push::<R1>(Share::new(R1(2, 65536, 0)));
        res_mgr.push::<R1>(Share::new(R1(3, 1572864, 0)));

        res_mgr.push::<R3>(Share::new(R3(1156469915, 32)));
        res_mgr.push::<R2>(Share::new(R2(940515885, 32)));
        res_mgr.push::<R3>(Share::new(R3(2294013149, 32)));
        res_mgr.push::<R2>(Share::new(R2(1197965350, 32)));
        res_mgr.push::<R3>(Share::new(R3(2418548769, 32)));
        res_mgr.push::<R2>(Share::new(R2(203047359, 32)));
    
        res_mgr.push::<R1>(Share::new(R1(4, 4194304, 0)));
        res_mgr.push::<R1>(Share::new(R1(5, 4194304, 0)));
        res_mgr.push::<R1>(Share::new(R1(6, 4194304, 0)));
        res_mgr.push::<R1>(Share::new(R1(7, 4194304, 0)));
        res_mgr.push::<R1>(Share::new(R1(8, 4194304, 0)));
        res_mgr.push::<R1>(Share::new(R1(9, 4194304, 0)));
