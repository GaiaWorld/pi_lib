...
impl<'b, 'a: 'b, T: Send + Sync + 'static, B: Send + Sync + 'static> TestStruct<'b, 'a, T, B> {
    ...
    ///
    #[pi_js_export]
    pub fn new(x: &'a T, y: &'b T, z: T, vec: Vec<B>) -> Self {
        TestStruct {
            x,
            y,
            z,
    ...
    pub fn flush<'a, X, Y>(mut self, x: X, y: &'a X, z: &'a mut Y, _c: Vec<X>, _: HashMap<X, Y>) -> Result<&'a mut Y, String>
...
impl<'b, 'a: 'b, T: Send + Sync + 'static, B: Send + Sync + 'static> TestStruct<'b, 'a, T, B> {
    ...
    ///
    #[pi_js_export(X = type(bool, usize, String), Y = type(bool, usize, String))]
    pub async fn sync<'a, X, Y>(mut self, x: X, y: &'a X, z: &'a mut Y, _c: Vec<Vec<X>>, _r: HashMap<&'a Vec<X>, Vec<&'a Y>>) -> Result<HashMap<&'a Vec<X>, Vec<&'a Y>>, String>
        where X: Clone, Y: Clone {
        Ok(_r)
    }
}
