...
impl ComponentData {
    pub fn from(name: String, is_must: bool) -> ComponentData{
        ComponentData{
            ...
            point_name: point_name(name.clone()),
...
impl Field {
    pub fn from(field: syn::Field, index: usize) -> Field{
        let mark = if is_component(&field) || is_enum_component(&field) {
            let field_ty_str1 = field.ty.clone().into_token_stream().to_string();
            let field_ty_str = unsafe{field_ty_str1.get_unchecked(0..field_ty_str1.len() -5)}.to_string();
...
impl Fields {
    pub fn from(fields: &syn::punctuated::Punctuated<syn::Field, syn::token::Comma>, ty: FieldsType) -> Fields{
...
pub struct VariantFields{
    ...
    pub point_names: Vec<syn::Ident>, //指针名
...
impl VariantFields {
    pub fn from(fields: &syn::punctuated::Punctuated<syn::Field, syn::token::Comma>, ty: FieldsType) -> VariantFields{
        ...
        let mut point_names: Vec<syn::Ident> = Vec::new(); //指针名
        ...
        for field in fields.iter(){
            ...
            let ComponentData{group_name, point_name, write_ref_name, read_ref_name, is_must:_, c_type: _} = ComponentData::from(field_ty_str, true);
            keys.push(key);
    ...
    pub fn from(variant: &syn::Variant) -> Variant{
        let fields = {
            match &variant.fields {
                syn::Fields::Named(field) => VariantFields::from(&field.named, FieldsType::Named),
                syn::Fields::Unnamed(field) => VariantFields::from(&field.unnamed, FieldsType::Unnamed),
                _ => panic!("enum error"),
            }
        };
        Variant{
...
//                 syn::Fields::Named(ref data)=> Fields::from(&data.named, FieldsType::Named),
//                 syn::Fields::Unnamed(ref data) => Fields::from(&data.unnamed, FieldsType::Unnamed),
