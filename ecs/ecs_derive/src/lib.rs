...
/// #[component(VecMap)] //  `VecMap` is a data structure for a storage component, This line is optional, defaults to `VecMap`
...
fn impl_write(ast: &DeriveInput, generics: &syn::Generics) -> proc_macro2::TokenStream {
    ...
    quote! {
        pub trait #write_trait_name<#name #ty_generics> {
            #trait_def
        }
            ...
            syn::Data::Struct(s) => {
                ...
                match fields {
                    syn::Fields::Named(fields) => {
                        ...
                        // modify
                        tokens.extend(quote! {
                            fn modify<F: Fn(&mut #name)>(&mut self, callback: F) {
                                callback(self.value);
