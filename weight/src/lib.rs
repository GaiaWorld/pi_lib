...
impl<T: Ord + fmt::Debug> fmt::Debug for WeightItem<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_tuple("WeightItem").field(&self.weight).field(&self.el).finish()
    }
}
impl<T: Ord + Clone> Clone for WeightItem<T> {
    fn clone(&self) -> Self {
        WeightItem{
            weight: self.weight,
            amount: self.amount,
...
// Ord trait所需
impl<T: Ord> Eq for WeightItem<T> {}
// Ord trait所需
impl<T: Ord> PartialEq for WeightItem<T> {
    fn eq(&self, other: &Self) -> bool {
        self.weight.eq(&other.weight)
    }
}
impl<T: Ord> PartialOrd for WeightItem<T> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        self.weight.partial_cmp(&other.weight)
    }
}
impl<T: Ord> Ord for WeightItem<T> {
    fn cmp(&self, other: &Self) -> Ordering {
        self.weight.cmp(&other.weight)
    }
}

impl<T: Ord> WeightItem<T> {
...
pub type DeletableHeap<T> = ExtHeap<WeightItem<T>>;

/// 增加可删除堆的操作接口
pub trait HeapAction<T: Ord, I> {
    /// 弹出指定权重的元素，并维护权重统计
    fn pop_weight(&mut self, weight: usize) -> Option<WeightItem<T>>;
    /// 移除指定位置的元素，并维护权重统计
    fn remove_weight(&mut self, index: usize) -> WeightItem<T>;
    /// 修改指定位置的元素，并维护权重统计
    fn modify_weight(&mut self, index: usize, weight: usize);
    /// 放入元素并维护权重统计
    fn push_weight(&mut self, item: WeightItem<T>);
}

impl<T: Ord + fmt::Debug, I> HeapAction<T, I> for ExtHeap<WeightItem<T>> {
    fn pop_weight(&mut self, weight: usize) -> Option<WeightItem<T>> {
