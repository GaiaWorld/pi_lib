...
use fnv::FnvHashMap;
use netstat2::{
    get_sockets_info, iterate_sockets_info, AddressFamilyFlags, ProtocolFlags, ProtocolSocketInfo,
    SocketInfo, TcpState,
};
use sysinfo::{DiskExt, NetworkExt, ProcessExt, ProcessStatus, ProcessorExt, System, SystemExt};
...
pub fn server_ports() -> Option<Vec<u16>> {
    ...
    Some(keys.map(|key| key.clone()).collect())
}
...
impl SysStat {
    ...
    //获取指定进程的网络连接状态
    pub fn current_process_sockets_info(
        &self,
        pid: i32,
        ip_type: NetIPType,
        protocol_type: NetProtocolType,
...
//过滤网络连接参数
fn filter_sockets_args(
    ip_type: NetIPType,
    protocol_type: NetProtocolType,
) -> (AddressFamilyFlags, ProtocolFlags) {
