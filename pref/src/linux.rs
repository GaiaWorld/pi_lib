...
impl SysSpecialStat for LinuxSysStat {
    fn sys_cpu_runnable(&self) -> (i32, i32) {
        if let Ok(info) = system::loadavg() {
            return (info.runnable,          //当前可用cpu逻辑核心数
                    info.total_runnable);   //可用cpu逻辑核心总数
        }
    ...
    fn sys_cpu_detal(&self) -> (f64, f64, f64, f64, f64, f64, f64, f64, f64, f64) {
        if let Ok(info) = system::cpu_times_percent(self.interval) {
            return (info.user,          //用户占用率，包括虚拟环境中虚拟cpu占用率
    ...
    fn process_current_detal(&self) -> (u32, u32, i64, i64, f64, f64, u64, i64, u64, u64, u64, u64, u64, i32, i64, f64, String, String) {
        if let Ok(info) = process::Process::new(self.process_current_pid()) {
            let (sys_usage, user_usage) = get_cpu_usage_by_process(&info);
            return (info.uid,                   //进程所属用户id
                    info.gid,                   //进程所属组id
...
//获取进程在内核态和用户态的cpu占用率
fn get_cpu_usage_by_process(process: &process::Process) -> (f64, f64) {
    ...
    (100 * (end_process_system - start_process_system) / (end_total_system - start_total_system) as f64,
     100 * (end_process_user - start_process_user) / (end_total_user - start_total_user) as f64);
}

//获取系统和进程在内核态和用户态的cpu占用时间
fn get_cpu_args(process: &process::Process) -> (u64, u64, f64, f64) {
    if let Ok(sys) = system::cpu_times() {
        return (sys.system,                         //系统内核态cpu占用时间，单位秒
                sys.user + sys.nice,                //系统用户态cpu占用时间，单位秒
                process.stime + process.cstime,     //进程内核态cpu占用时间，单位秒
                process.utime + process.cutime)     //进程用户态cpu占用时间，单位秒
    }
