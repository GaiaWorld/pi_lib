...
pub struct ComputeResult {
    pub size: Size<f32>,
    pub children: Vec<result::Layout>,
}

struct FlexItem<'a> {
    node: &'a Rc<RefCell<InternalNode>>,
...
fn round_layout(layout: &mut result::Layout, abs_x: f32, abs_y: f32) {
    ...
    layout
        .children
        .iter_mut()
        .for_each(|child| round_layout(child, abs_x, abs_y));
}
...
fn compute_internal(
    ...
    if node.children.is_empty() {
        ...
        if let Some(ref measure) = node.measure {
            ...
            node.layout_cache = Some(result::Cache {
                node_size,
                parent_size,
                perform_layout,
                result: result.clone(),
    ...
    flex_lines.iter_mut().try_for_each(|line| -> Result<()> {
        ...
        line.items.iter_mut().try_for_each(|child| -> Result<()> {
            ...
            if (child.node.borrow().style.flex_grow == 0.0
                && child.node.borrow().style.flex_shrink == 0.0)
        ...
        loop {
            ...
            let sum_flex_grow: f32 = unfrozen
                .iter()
                .map(|item| item.node.borrow().style.flex_grow)
                .sum();
            let sum_flex_shrink: f32 = unfrozen
                .iter()
                .map(|item| item.node.borrow().style.flex_shrink)
                .sum();

            let free_space = if growing && sum_flex_grow < 1.0 {
                (initial_free_space * sum_flex_grow as f32)
                    .maybe_min(node_inner_size.main(dir) - used_space)
            } else if shrinking && sum_flex_shrink < 1.0 {
                (initial_free_space * sum_flex_shrink as f32)
                ...
                if growing && sum_flex_grow > 0.0 {
                    unfrozen.iter_mut().for_each(|child| {
                        child.target_size.set_main(
                            ...
                            child.flex_basis
                                + free_space
                                    * (child.node.borrow().style.flex_grow / sum_flex_grow) as f32,
                        );
                    });
                    ...
                    let sum_scaled_shrink_factor: f32 = unfrozen
                        .iter()
                        .map(|child| {
                            child.inner_flex_basis * child.node.borrow().style.flex_shrink as f32
                        })
                        .sum();

                    if sum_scaled_shrink_factor > 0.0 {
                        unfrozen.iter_mut().for_each(|child| {
                            let scaled_shrink_factor = child.inner_flex_basis
                                * child.node.borrow().style.flex_shrink as f32;
                            child.target_size.set_main(
                                dir,
                                child.flex_basis
                                    + free_space
            ...
            line.items.iter_mut().try_for_each(|child| -> Result<()> {
                ...
                child.baseline = calc_baseline(&result::Layout {
                    order: node
                        .children
                        .iter()
                        .position(|n| Rc::ptr_eq(n, child.node))
                        .unwrap() as u32,
                    size: result.size,
                    location: Point { x: 0.0, y: 0.0 },
                    children: result.children,
