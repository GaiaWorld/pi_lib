...
fn main() {
    ...
    let mut fixtures: Vec<_> = fixtures
        ...
        .filter_map(|a| a.ok())
        .filter(|f| {
            f.path().is_file() && f.path().extension().map(|p| p == "html").unwrap_or(false)
        })
        .map(|f| {
    ...
    .unwrap();
    fs::write(
        repo_root.join("tests").join("generated").join("mod.rs"),
        test_mods.to_string(),
    )
    .unwrap();
...
fn generate_assertions(index: &mut usize, ident: &str, node: &json::JsonValue) -> TokenStream {
        ...
        json::JsonValue::Array(ref value) => {
            if value.len() > 0 {
                ...
                let body = value
                    ...
                    .map(|(i, child)| {
                        *index += 1;
                        generate_node(p, index, &format!("{}{}", ident, i + 2), child)
                    })
                    .collect();
                body
            } else {
