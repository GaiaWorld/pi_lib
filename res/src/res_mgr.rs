...
impl ResMgr {
    ...
    pub fn register<T: Res + 'static>(&mut self, configs: [usize; 9]) {
        let map = Share::new(ResMap::<T>::with_config(&configs));
        ...
        self.tables.insert(TypeId::of::<T>(), (map, arr));
    }
    ...
    pub fn collect<T: Res + 'static>(&mut self, now: usize) {
        ...
        for v in self.tables.values() {
            ...
            let mut i = 0;
            for ss in arr.iter() {
                let calc_max = capacity * v.1[i] / self.weight; // 该lru根据权重算出来的可增加的内存总量，如果加上min_capacity则是最大容量max_capacity
                match ss {
